...
export default function AppShell({ children, activeTab, onTabChange }: AppShellProps) {
    ...
    const handleTabClick = (tab: Tab) => {
        if (onTabChange) {
            ...
            // If no handler provided (e.g. on blog pages), navigate home with state
            // Note: In a real app we might use query params or context, but for now we'll just nav home
            // and let the user navigate from there, or we could pass query params.
            // For simplicity in this refactor, if we are on a blog page, clicking a tab goes home.
            router.push("/");
        }
    };
