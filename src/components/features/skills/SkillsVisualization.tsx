...
export default function SkillsVisualization({
  ...
  // Calculate radar data - regenerate when skills change
  const radarData = useMemo(() => {
    const data = calculateCategoryScores(skills, skillCounts);
    return data;
  }, [skills, skillCounts]);
  ...
  // Check if viewing filtered data
          ...
          <CardContent className="p-0 text-center">
            ...
            {isFiltered && onClearFilter && (
              <button
                ...
                className="mt-4 px-4 py-2 text-sm font-medium text-sky-600 dark:text-sky-400 hover:text-sky-700 dark:hover:text-sky-300 border border-sky-300 dark:border-sky-600 rounded-lg hover:bg-sky-50 dark:hover:bg-sky-900/30 transition-colors"
              >
                View All Skills
              </button>
            )}
  ...
  return (
    <div className="space-y-8">
      ...
      {isFiltered && (
        <div className="flex items-center justify-between p-3 sm:p-4 bg-sky-50 dark:bg-sky-900/20 border border-sky-200 dark:border-sky-800 rounded-xl">
          <div className="flex items-center gap-2">
            <span className="text-sky-600 dark:text-sky-400">🔍</span>
            <span className="text-sm text-slate-700 dark:text-slate-300">
              Showing skills from <span className="font-semibold text-sky-600 dark:text-sky-400">{selectedIssuer}</span> certifications
              {certCount !== undefined && (
                <span className="text-slate-500 dark:text-slate-400"> ({certCount} cert{certCount !== 1 ? 's' : ''})</span>
              )}
            </span>
          </div>
          {onClearFilter && (
            <button
              onClick={onClearFilter}
              className="text-xs sm:text-sm font-medium text-sky-600 dark:text-sky-400 hover:text-sky-700 dark:hover:text-sky-300 px-2 sm:px-3 py-1 rounded-lg hover:bg-sky-100 dark:hover:bg-sky-900/50 transition-colors"
            >
              Clear Filter ✕
            </button>
          )}
      ...
      <div className="space-y-8">
        {/* Top Section - Radar Chart and Category Buttons */}
        ...
        {/* Selected Category Skills Detail - Centered on desktop */}
